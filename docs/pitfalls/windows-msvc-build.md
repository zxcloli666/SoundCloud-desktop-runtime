...
реально выполняет JS. Архитектурный блокер снят. `crates/rn-windows/`
— тот же движок, что и `rn-linux`, под своим бинарём (winit/glutin/
skia-safe кроссплатформенны сами по себе, ни строчки платформенного кода
в самом крейте) — собран и подтверждён рендером на этом же Linux-хосте;
живой прогон `rn-windows.exe` именно на Windows-машине (а не только
сборка Hermes под MSVC, которая уже проверена выше) отдельно в рамках
этого скаффолда не переделывался.
